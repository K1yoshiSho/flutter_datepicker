...
class DateRangePickerHelper {
  ...
  static bool isRangeEquals(dynamic range1, dynamic range2) {
    ...
    if (range1 == range2 ||
        (isSameDate(range1.startDate, range2.startDate) && isSameDate(range1.endDate, range2.endDate))) {
      return true;
    }
  ...
  static dynamic getNextViewStartDate(
      DateRangePickerView pickerView, int numberOfWeeksInView, dynamic date, bool isRtl, bool isHijri) {
    if (isRtl) {
  ...
  static bool isDateAsCurrentMonthDate(
      dynamic visibleDate, int rowCount, bool showLeadingAndTrialingDates, dynamic date, bool isHijri) {
  ...
  static Map<String, double> getTopAndLeftValues(
      bool isRtl, double left, double top, double cellWidth, double cellHeight, double width) {
  ...
  static bool canMoveToPreviousViewRtl(DateRangePickerView view, int numberOfWeeksInView, dynamic minDate,
      dynamic maxDate, List<dynamic> visibleDates, bool isRtl, bool enableMultiView, bool isHijri) {
    if (isRtl) {
  ...
  static bool canMoveToPreviousView(DateRangePickerView view, int numberOfWeeksInView, dynamic minDate,
      List<dynamic> visibleDates, bool enableMultiView, bool isHijri) {
    switch (view) {
      case DateRangePickerView.month:
        {
  ...
  static bool canMoveToNextView(DateRangePickerView view, int numberOfWeeksInView, dynamic maxDate,
      List<dynamic> visibleDates, bool enableMultiView, bool isHijri) {
    switch (view) {
      case DateRangePickerView.month:
        {
          if (!isHijri && numberOfWeeksInView != 6) {
            ...
            if ((nextDate.month > maxDate.month == true && nextDate.year == maxDate.year) ||
  ...
  static bool isBetweenMinMaxDateCell(
      dynamic date, dynamic minDate, dynamic maxDate, bool enablePastDates, dynamic view, bool isHijri) {
    ...
    if (pickerView == DateRangePickerView.year) {
      return ((date.month >= minDate.month == true && date.year == minDate.year) || date.year > minDate.year == true) &&
          ...
          (enablePastDates ||
              (!enablePastDates &&
                  ((date.month >= today.month == true && date.year == today.year) || date.year > today.year == true)));
    } else if (pickerView == DateRangePickerView.decade) {
